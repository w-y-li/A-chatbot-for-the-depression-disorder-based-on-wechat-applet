    ...
    for i in range(len(pred)):
        ...
        if max_index == 4:
            ...
            if dis1 > 0.9:
                ...
                continue_results.append((max_index+1) * dis1 + max_index * dis2)
        else:
            dis1 = math.exp(pred[i][max_index]) / (math.exp(pred[i][max_index]) + math.exp(pred[i][max_index + 1]))
            dis2 = math.exp(pred[i][max_index + 1]) / (math.exp(pred[i][max_index]) + math.exp(pred[i][max_index + 1]))
            if dis1 > 0.9:
                ...
                continue_results.append(max_index + 2)
            else:
                continue_results.append((max_index + 1) * dis1 + (max_index + 2) * dis2)
...
def scorepre(text, qnum):
    ...
    for batch_index, batch_con in enumerate(xDataloader):
        ...
        # print(pred)
        classname.append(text_class_name(pred))
    end = time.time()
    # print(f"耗时为：{end - start} s")
    return classname
...
def getpoint():
    ...
    for i in range(1, 21):
        sdspoint[f'{i}'] = 0
    sdsans_file = open(r".\..\txt\sds2.16.txt", "r", encoding='utf-8-sig')
    for line in sdsans_file.readlines():
        if line == '':
            break
        line=line.split(':',1)
        tmp=int(line[0])
        sdspoint[line[0]] = float(scorepre([line[1].strip()], tmp)[0][0])
    return sdspoint
