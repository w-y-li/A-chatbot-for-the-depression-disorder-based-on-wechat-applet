...
class TextCnnModel(nn.Module):
    def __init__(self):
        ...
        self.num_filter_total = parsers(0).num_filters * len(parsers(0).filter_sizes)
        self.Weight = nn.Linear(self.num_filter_total, parsers(0).class_num, bias=False)
        self.bias = nn.Parameter(torch.ones([parsers(0).class_num]))
        self.filter_list = nn.ModuleList([
            nn.Conv2d(1, parsers(0).num_filters, kernel_size=(size, parsers(0).hidden_size)) for size in parsers(0).filter_sizes
        ])
    ...
    def forward(self, x):
        ...
        for i, conv in enumerate(self.filter_list):
            ...
            maxPool = nn.MaxPool2d(
                kernel_size=(parsers(0).encode_layer - parsers(0).filter_sizes[i] + 1, 1)
            )
            # maxPool: [batch_size, channel=2, 1, 1]
            pooled = maxPool(out).permute(0, 3, 2, 1)  # [batch_size, h=1, w=1, channel=2]
            pooled_outputs.append(pooled)

        h_pool = torch.cat(pooled_outputs, len(parsers(0).filter_sizes))  # [batch_size, h=1, w=1, channel=2 * 3]
...
class BertTextModel_encode_layer(nn.Module):
    def __init__(self):
        ...
        self.bert = BertModel.from_pretrained(parsers(0).bert_pred)

        for param in self.bert.parameters():
            param.requires_grad = True

        self.linear = nn.Linear(parsers(0).hidden_size, parsers(0).class_num)
...
class BertTextModel_last_layer(nn.Module):
    def __init__(self):
        ...
        self.convs = nn.ModuleList(
            [nn.Conv2d(in_channels=1, out_channels=parsers(0).num_filters, kernel_size=(k, parsers(0).hidden_size),) for k in parsers(0).filter_sizes]
        )
        self.dropout = nn.Dropout(parsers(0).dropout)
        self.fc = nn.Linear(parsers(0).num_filters * len(parsers(0).filter_sizes), parsers(0).class_num)
    ...
    def forward(self, x):
        ...
        out = hidden_out[0].unsqueeze(1)   # shape [batch_size, 1, max_len, hidden_size]
        out = torch.cat([self.conv_pool(out, conv) for conv in self.convs], 1)  # shape  [batch_size, parsers(0).num_filters * len(parsers(0).filter_sizes]
