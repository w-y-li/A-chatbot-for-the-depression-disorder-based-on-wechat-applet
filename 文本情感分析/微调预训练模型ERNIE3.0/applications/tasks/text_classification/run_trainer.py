...
def model_from_params(params_dict, dataset_reader):
    ...
    if params_dict.get("need_mask_id", False):
        mask_id = dataset_reader.train_reader.dataset.fields[0].field_reader.tokenizer.vocab["[MASK]"]
        params_dict["mask_id"] = mask_id
...
if __name__ == "__main__":
    ...
    param_dict = params.from_file('./examples/cls_ernie_fc_ch.json')
    _params = params.replace_none(param_dict)

    # 记得import一下注册的模块
    register.import_modules()

    register.import_new_module("model", "ernie_classification")
    register.import_new_module("model", "bow_classification")
    register.import_new_module("model", "multi_label_classification")
    register.import_new_module("trainer", "custom_trainer")
    register.import_new_module("trainer", "custom_dynamic_trainer")
    register.import_new_module("reader", "multi_label_field_reader")
